...
export const prisma = new PrismaClient({ adapter, log: ["query","info", "error", "warn"] // 쿼리 로그, 에러 로그, 경고 로그를 모두 출력하도록 설정
});
...
export const pool = mysql.createPool({
  ...
  database: process.env.DB_NAME || "umc_practice", // 데이터베이스 이름
  password: process.env.DB_PASSWORD || "synctlover0504#", // 비밀번호
