...
export const createReview = async (storeId: number, memberId: number, data: AddReviewRequest): Promise<AddReviewResponse> => {
  const store = await getStoreById(storeId);
  if (!store) throw new StoreNotFoundError("존재하지 않는 가게입니다.", { storeId });
  const reviewId = await addReview(storeId, memberId, data);
  return { reviewId };
};
...
export const listStoreReviews = async (storeId: number, cursor: number): Promise<ReviewsResponse> => {
  ...
  return {
    ...
    pagination: { cursor: last ? Number(last.reviewId) : null },
  };
};
